...
const automaticResultObservation = (outcome) => {
  if (!outcome) return 'Aguardando leitura do documento escolar.';
  const responses = `${outcome.answeredCount || 0}/${outcome.totalItems || 0} respostas registradas`;
  if (!outcome.canScore) return `${responses}. O documento nao apresentou regra suficiente para pontuacao, maximo ou porcentagem; manter as informacoes parciais para revisao profissional.`;
  return `${responses}. Calculo baseado em ${outcome.sourceLabel?.toLowerCase() || 'regra registrada'}. Resultado preenchido automaticamente e pendente de revisao profissional.`;
};
...
const makeAutomaticResult = (questionnaire, outcome) => ({
  ...
  observation: automaticResultObservation(outcome),
});
...
function EvaluationForm({ initialApplication, onSaved }) {
  ...
  const handleSubmit = (event, status) => {
    ...
    const generatedResult = questionnaire && automaticOutcome ? makeAutomaticResult(questionnaire, automaticOutcome) : null;
    const questionnaireResult = automaticOutcome?.canScore && automaticOutcome.complete ? { ...generatedResult, id: `questionnaire-${Date.now()}`, autoGenerated: false } : null;
    const manualQuestionnaireResult = questionnaire && !automaticOutcome?.canScore && answeredCount === questionnaire.items.length ? { ...generatedResult, id: `questionnaire-${Date.now()}`, autoGenerated: false } : null;
    const payload = {
      ...form,
